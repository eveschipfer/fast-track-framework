...
async def test_repository_injection(app: FastTrackFramework) -> None:
    ...
    @app.get("/test-repo")
    async def test_route(repo: UserRepository = Inject(UserRepository)) -> dict:
...
async def test_create_user_via_http(app: FastTrackFramework) -> None:
    ...
    @app.post("/users")
    async def create_user(repo: UserRepository = Inject(UserRepository)) -> dict:
...
async def test_read_user_via_http(app: FastTrackFramework) -> None:
    ...
    async def get_user(
        user_id: int, repo: UserRepository = Inject(UserRepository)
    ) -> dict:
